// Code qui affiche : 
// - toutes les époques
// - les personnages classés par époque
// - les fiches des personnages correspondant (informations dynamiques, provenant de la BDD)
	...
	function loadListeEpoque() {
		...
		//appel Ajax pour afficher toutes les époques 
		$.ajax({
			type: 'GET',
			...
			success: function(data) {
				$.each(data, function(i,item){
					...
					//la valeur de l'id du lien (a) correspond à l'id de l'époque dans la table 
					ListeEpoque.append('<li><a id="'+item.id+'" href="#" rel="external">'+item.epoque+'<p>'+item.debut_epoque+' '+item.fin_epoque+'</p><img src="http://argosapps.fr/retro-appli-accordeon-fiches/img/arrow_down.png" alt="icone flèche" class="arrow_icon"/></a><ul  class="perso-epoque-liste" data-role="listview" data-icon="false"></ul></li>');	
					$('#ul_liste_epoque').listview('refresh');
					
				});
				
				
					//au clic sur une des époques, on va récupérer les personnages qui correspondent à cette époque
				 $('#ul_liste_epoque li a').click(function(e) {
				 if ( $(this).hasClass("opened") == true) {
						$('#ul_liste_epoque li ul').hide();
						$('#ul_liste_epoque li a').removeClass("opened");
				 ...
				 //on cache tous les sous-listes (personnages d'une épque)
				 $('#ul_liste_epoque li ul').hide();
				 
						
							//on récupère la valeur de l'id du lien sur lequel l'utilisateur a cliqué, et on la stocke dans la variable idepoqueperso
							var idepoqueperso = $(this).attr('id');
						...
						$.ajax({
								type: "GET",
								dataType: 'jsonp',
								url: 'http://www.argosapps.fr/retro_php_server/persos-selon-epoque.php?callback=?',
								data: ({'idepoqueperso' : idepoqueperso}), //on passe la variable en data pour la récupérer en PHP par $_GET['idepoqueperso']
								success: function(data){
									...
									$.each(data, function(i, item) {				
										...
										ulduntype.append('<li class="ui-li-has-thumb ui-btn-icon-right ui-li-has-arrow ui-li ui-btn-up-c remove-btn"><a class="ui-btn" id="'+idpersoduntype+'" href="#" rel="external"><img src="'+iconepersodutype+'" alt=""/>'+item.nom+' '+item.prenom+'</a><div id="fiche-perso-epoque-'+idpersoduntype+'"></div></li>');
										
									});
									
										//au clic sur un des personnages d'un type, on va récupérer les infos de ce lieu pour afficher sous forme de fiche
										$('#ul_liste_epoque li ul li a').click(function(e) {
												//on va masquer tous les div au départ(contenant les fiches persos)
											$('#ul_liste_epoque li ul li div').hide();
													...
													$.ajax({
														...
														success: function(data){
															...
															$.each(data, function(i, item) {
																		...
																		var $divperso = '#fiche-perso-epoque-' + idperso;
																		$($divperso).show();
																		//on affiche le code html des fiches personnages :
																		$($divperso).html("<div id='contenu-fiche' data-role='content'>"+
																		...
																		"<!--Lieu fréquenté-->"+
																			...
																			$("#ul_liste_epoque li ul li div #contenu-fiche").trigger('create');
																			
																			//on stocke les infos dynamiques des personnages
																			var prenom = item.prenom; 
																			var nom = item.nom; 
																			...
																			//on affiche ces infos dynamiques aux endroits voulus :
																				...
																				$("#ul_liste_epoque li ul li div #contenu-fiche").trigger('create'); //rafraîchir styles JQM
								...
								error: function(){
									//do your thing
									console.log('Echec infos persos');
								}
								});
			...
			error: function(data) {
				ListeEpoque.append('<li>Impossible de charger les époques. Veuillez vérifier votre connexion réseau !');
			}
